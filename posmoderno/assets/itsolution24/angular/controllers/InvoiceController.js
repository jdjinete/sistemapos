...
function (
    ...
    $scope.applyFilter = function() {
        var startDate = $scope.filter.startDate ? encodeURIComponent($scope.filter.startDate) : '';
        var endDate = $scope.filter.endDate ? encodeURIComponent($scope.filter.endDate) : '';
        var url = 'invoice.php?startDate=' + startDate + '&endDate=' + endDate;
    ...
    $("#invoice-invoice-list").dataTable({
        ...
        "dom": "lfBrtip",
        "serverSide": true,
        "ajax": API_URL + "/_inc/invoice.php?from="+$startDate+"&to="+$endDate+"&type="+$type+"&customer_id="+$customerId,
        "fixedHeader": true,
        "order": [[ 0, "desc"]],
        "aLengthMenu": [
            [10, 25, 50, 100, 200, -1],
